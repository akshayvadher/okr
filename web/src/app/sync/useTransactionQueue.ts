...
import {
  ...
  KeyResult,
  Objective, UpdateProgressRequestWithKeyResult
} from "@/types";
...
export const useTransactionQueue = () => {
  ...
  const transactionApi = useCallback(async (transaction: TransactionEnriched) => {
    api.addTransaction(transaction).then();
  }, [])

  const processInMemory = useCallback(async (
      id: string,
  ...
  }, [doesTransactionExist, transactionLocalDbProcessor, transactionLocalInMemoryProcessor])


  const transactionLocalDb = useCallback(async (transaction: TransactionEnriched) => {
    const {exists} = await doesTransactionExist(transaction.id)
    if (exists) return;
    if (!db) throw new Error("db connection not available");
    db.exec(`
  ...
  }, [complete, fail, markAsProcessing, transactionApi, transactionLocalDb, transactionLocalDbProcessor, transactionLocalInMemoryProcessor]);
